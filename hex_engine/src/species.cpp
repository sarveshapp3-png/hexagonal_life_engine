...
Species::Species(std::string n, std::shared_ptr<Genome> g, uint32_t tick)
    : name(n), genome(g), start_tick(tick) {
...
std::shared_ptr<Species> FossilRecord::record_new_species(std::shared_ptr<Genome> genome, std::shared_ptr<Species> ancestor, uint32_t tick) {
    (void)ancestor; // In the future, we can track lineage
    std::string name = generate_name();
    auto species = std::make_shared<Species>(name, genome, tick);
...
void FossilRecord::update_stats(uint32_t tick, uint32_t total_orgs) {
    (void)tick;
    (void)total_orgs;
    // Remove extinct species from extant list
